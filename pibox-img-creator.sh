...
END_OF_CMD
	...
	sudo e2fsck -f ${loopdev}p2 || fail "Unable to fsck / partition"
	...
	$VIRTUAL_ENV/bin/python $ROOT/setup_image.py "$build_img" "${QEMU_PATH}" "${QEMU_RAM}"
...
function find_qemu {
	...
	local qemu_bins=`find $ROOT -name $QEMU_BINARY`
	local sys_qemu=`command -v $QEMU_BINARY`
	if [ $? -eq 0 ] ; then
		qemu_bins="${qemu_bins}${sys_qemu}"
	fi

	if [ `echo $qemu_bins | wc -l` -gt 0 ] ; then
